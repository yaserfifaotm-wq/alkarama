...
let appState = {
    ...
    adminPasscode: '19961996', // الرمز الافتراضي لحذف الإجازات (المسؤول)
    disableTimeRestrictions: false, // لتعطيل قيود وقت الحجز للمسؤول
    isAdminMode: false, // حالة تسجيل دخول المسؤول للحجز وتفادي القيود بالكامل
...
// قائمة أسماء موظفي مركز الكرامة الرسمية (81 موظف)
const OFFICIAL_EMPLOYEES = [
    ...
    "زهراء صاحب محمود",
    "غادة باسم عبد الحسن",
    "فاطمة ضرغام عبد الرضا",
    "خديجة مهدي صالح",
    "امير صبحي عطية",
    "زهراء عطا الله دخيتة",
    "رشا كاظم محمد",
    "سالي عامر ياسين",
    "زيد عقيل كاظم",
    "حسنين يحيى شاكر",
    ...
    "هدى مهدي علي",
    "مثال عبد علي عودة",
    "ياسر حسين عبد المطلب",
    "ليلى حمودي عبد الامير",
    "نضال حسن فهد",
    "ياسر خضير عباس",
    "سعد رزاق كريم",
    "ضي احمد عبد ناطور",
    ...
    "نسيم فاروق امين",
    "بشرى كريم عبيد",
    "حسن توفيق عبد الامير",
    "علاء فرات عبد الرضا",
    "زهراء هادي عيسى",
    "سحر هادي سلمان",
    "سرى سعدون عبد الهادي",
    "نوردان شاكر محاو",
    "مصطفى محسن كاظم",
    "مصطفى كريم رحيم",
    "آلاء محمد عبود",
    "سارة محمد علي عباس",
    "منى محمد عطية",
    "صفاء عبد الكاظم جواد",
    "رؤى رعد صالح مهدي",
    "رجاء عبد الله عبود",
    "محمد الباقر نعيم كري",
    "حوراء حسين جواد",
    "مروة سامي عبد الكريم",
    "تبارك ضرغام جواد",
    "وسام عبد الكاظم",
    "لبنى حيدر جاسم",
    "ضرغام توفيق عبد الامير",
    "سعد عبد الحسين عبد الكريم",
    "زبيدة هادي عباس",
    "دانيا ظافر حسن",
    "ايمان غفار سعيد",
    "رفاه سلام فليح",
    ...
    "خولة رحيم ناصر",
    "حسام عادل نعمة",
    "ازهار عبد الحسن محمد",
    "صبا رحيم هادي",
    "حوراء حسين جاسم",
    "ارشد حسن علي"
];

// تهيئة وإعداد الاقتراحات التلقائية المخصصة (نافذة صغيرة أسفل حقل كتابة الاسم)
function setupEmployeesDatalist() {
    const nameInput = document.getElementById("employee-name");
    const suggestionsBox = document.getElementById("suggestions-box");
    if (!nameInput || !suggestionsBox) return;

    // إزالة السمة list لتفادي تداخل المقترحات الافتراضية للمتصفح
    nameInput.removeAttribute("list");

    let highlightedIndex = -1;
    let filteredList = [];

    // دالة لتحديث رسم المقترحات
    function renderSuggestions(list) {
        ...
        list.forEach((item, index) => {
            ...
            // عند النقر بالماوس أو اللمس على المقترح
    ...
    function selectSuggestion(value) {
        ...
        // نقل التركيز لحقل القسم تلقائياً تسهيلاً للمستخدم
        const deptInput = document.getElementById("employee-dept");
        if (deptInput) deptInput.focus();
    }

    // الاستماع للكتابة في حقل الاسم (تصفية من أول حرف)
    nameInput.addEventListener("input", () => {
        ...
        // تصفية الأسماء بناءً على المدخل المنظف
        const normalizedInput = normalizeArabicName(value);
        const matches = OFFICIAL_EMPLOYEES.filter(name => 
            normalizeArabicName(name).includes(normalizedInput)
        );

        renderSuggestions(matches);
    });

    // إظهار المقترحات أيضاً عند الضغط على الحقل إذا كان يحتوي على نص
    ...
    // التنقل بلوحة المفاتيح (الأسهم والإنتر)
    nameInput.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowDown") {
            ...
            renderSuggestions(filteredList);
            scrollToHighlighted();
            ...
            if (highlightedIndex >= 0 && highlightedIndex < filteredList.length) {
                e.preventDefault();
                selectSuggestion(filteredList[highlightedIndex]);
            }
    ...
    // تمرير القائمة تلقائياً إذا كان الاختيار خارج حدود النافذة المرئية للصندوق
    function scrollToHighlighted() {
        const activeItem = suggestionsBox.querySelector(".suggestion-item.highlighted");
        if (activeItem) {
            const containerHeight = suggestionsBox.clientHeight;
            const itemTop = activeItem.offsetTop;
            const itemHeight = activeItem.clientHeight;

            if (itemTop + itemHeight > suggestionsBox.scrollTop + containerHeight) {
                suggestionsBox.scrollTop = itemTop + itemHeight - containerHeight;
            } else if (itemTop < suggestionsBox.scrollTop) {
                suggestionsBox.scrollTop = itemTop;
            }
        }
    }

    // إغلاق المقترحات عند النقر خارج حقل الاسم أو صندوق المقترحات
    document.addEventListener("click", (e) => {
        if (e.target !== nameInput && e.target !== suggestionsBox && !suggestionsBox.contains(e.target)) {
            suggestionsBox.classList.add("d-none");
            highlightedIndex = -1;
        }
    });
...
function isBookingWindowOpen(targetDateStr, now) {
    if (appState.disableTimeRestrictions || appState.isAdminMode) {
...
// تفعيل وضع المسؤول (بدون المطالبة بالرمز لأنه تم التحقق منه مسبقاً)
function enterAdminMode() {
    appState.isAdminMode = true;
    
    // إظهار شريط المسؤول بالأعلى
    const banner = document.getElementById("admin-mode-banner");
    if (banner) {
        banner.classList.remove("d-none");
    }
    
    // تحديث تفاصيل اليوم المعروض لفتح الحجز فوراً
    if (appState.selectedDate) {
        renderDayDetails(appState.selectedDate);
    }
}

// المطالبة برمز المرور وتفعيل وضع المسؤول (للحجز الاستثنائي)
function enterAdminModePrompt() {
    const password = prompt("يرجى إدخال رمز مرور المسؤول لتفعيل وضع الحجز الاستثنائي:");
    ...
    alert("تم تفعيل وضع المسؤول بنجاح! تم إلغاء كافة قيود وقت الحجز والحدود الشهرية.");
    enterAdminMode();
}

// إلغاء وضع المسؤول للخروج
function exitAdminMode() {
    appState.isAdminMode = false;
    
    // إخفاء شريط المسؤول بالأعلى
    const banner = document.getElementById("admin-mode-banner");
    if (banner) {
        banner.classList.add("d-none");
    }
    
    // تحديث تفاصيل اليوم المعروض لإعادة تطبيق القيود
    if (appState.selectedDate) {
        renderDayDetails(appState.selectedDate);
    }
    
    alert("تم الخروج من وضع المسؤول بنجاح.");
}
...
function loadSettings() {
    ...
    if (settings) {
        try {
            ...
            appState.adminPasscode = parsed.adminPasscode || '19961996';
            if (appState.adminPasscode === '1234') {
                appState.adminPasscode = '19961996'; // ترقية تلقائية للرمز الجديد
            }
...
function openSettingsModal() {
    if (!appState.isAdminMode) {
        ...
        // تفعيل وضع المسؤول عند كتابة الرمز بنجاح
        enterAdminMode();
    }

    // تعبئة البيانات
    document.getElementById("admin-passcode").value = appState.adminPasscode || '19961996';
...
function saveSystemSettings() {
    const passcode = document.getElementById("admin-passcode").value.trim() || '19961996';
...
function renderDayDetails() {
    ...
    document.getElementById("slots-count-text").textContent = `الشاغر: ${Math.max(remaining, 0)} / 8`;
    ...
    if (!windowStatus.isOpen) {
        ...
        // وقت الحجز متاح
        const maxAllowed = (appState.disableTimeRestrictions || appState.isAdminMode) ? 999 : 8;
        if (count >= maxAllowed) {
...
function handleBookingSubmit(event) {
    ...
    // 1. تنظيف واختبار اسم الموظف المدخل والبحث عن تطابق في القائمة الرسمية لتوحيد التهجئة
    let matchedOfficialName = null;
    const normalizedInputName = normalizeArabicName(name);
    
    for (const officialName of OFFICIAL_EMPLOYEES) {
        if (normalizeArabicName(officialName) === normalizedInputName) {
            matchedOfficialName = officialName;
            break;
        }
    }
    
    // 2. التحقق من صحة الاسم (إلا إذا كان وضع المسؤول نشطاً لتسجيل اسم جديد)
    if (!appState.isAdminMode && !matchedOfficialName) {
        alert("عذراً! الاسم المكتوب غير مسجل في قائمة موظفي المركز الرسمية. يرجى اختيار اسمك الثلاثي الصحيح من قائمة الاقتراحات المتاحة.");
        return;
    }
    
    const finalName = matchedOfficialName || name;
    const finalNormalizedName = normalizeArabicName(finalName);

    // 3. التحقق من تكرار الحجز لنفس الشخص في نفس اليوم
    const dayBookings = appState.bookings[targetDate] || [];
    const hasDuplicate = dayBookings.some(booking => normalizeArabicName(booking.name) === finalNormalizedName);
    if (!appState.isAdminMode && hasDuplicate) {
        alert(`عذراً! الموظف (${finalName}) لديه حجز مسجل بالفعل في هذا اليوم ولا يمكنه حجز إجازتين في نفس اليوم.`);
        return;
    }

    // 3.5 التحقق من تكرار الحجز من نفس الجهاز لهذا اليوم لمنع حجز موظف لزملائه
    if (!appState.isAdminMode && localStorage.getItem("alkarama_booked_" + targetDate) === "true") {
        alert("عذراً، لقد قمت بالفعل بتسجيل حجز لهذا اليوم من هذا الجهاز. يُسمح بالحجز لمرة واحدة فقط لكل موظف.");
        return;
    }

    // 4. التحقق من الحد الأقصى للإجازات شهرياً للموظف (3 إجازات كحد أقصى)
    const yearMonthStr = targetDate.substring(0, 7); // YYYY-MM
    const monthlyCount = getEmployeeMonthlyBookingsCount(finalName, yearMonthStr);
    if (!appState.disableTimeRestrictions && !appState.isAdminMode && monthlyCount >= 3) {
        const arabicMonthName = ARABIC_MONTHS[dateObj.getMonth()];
        alert(`عذراً! الموظف (${finalName}) لديه بالفعل ${monthlyCount} إجازات مسجلة في شهر ${arabicMonthName}. الحد الأقصى المسموح به هو 3 إجازات شهرياً.`);
        return;
    }
    ...
    const newBooking = {
        id: generateUniqueId(),
        name: finalName, // استخدام التهجئة الموحدة الرسمية
    ...
    if (appState.dbMode === 'firebase' && appState.isFirebaseInitialized && appState.dbRef) {
        ...
        dayRef.transaction((currentBookings) => {
            ...
            const maxAllowed = (appState.disableTimeRestrictions || appState.isAdminMode) ? 999 : 8;
            if (currentBookings.length >= maxAllowed) {
            ...
            if (error) {
                ...
                // نجاح الحجز
                localStorage.setItem("alkarama_booked_" + targetDate, "true");
                nameInput.value = "";
                notesInput.value = "";
                deptInput.value = "";
            }
...
function handleDeleteBooking(bookingId, employeeName) {
    if (!appState.isAdminMode) {
        ...
        // تأكيد حذف بسيط فقط لتجنب الضغط العرضي
        if (!confirm(`هل أنت متأكد من حذف حجز الموظف (${employeeName})؟`)) {
            return;
        }
    }
    ...
    if (appState.dbMode === 'firebase' && appState.isFirebaseInitialized && appState.dbRef) {
        ...
        dayRef.transaction((currentBookings) => {
            ...
            if (error) {
                ...
                // تم الحذف
                localStorage.removeItem("alkarama_booked_" + targetDate);
            }
        });
    } else {
        // حذف محلي
