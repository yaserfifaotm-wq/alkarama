...
function openSettingsModal() {
    // تعبئة الراديو
    if (appState.dbMode === 'firebase') {
        document.getElementById("mode-firebase-radio").checked = true;
        document.getElementById("firebase-config-fields").classList.remove("d-none");
    } else {
        document.getElementById("mode-local-radio").checked = true;
        document.getElementById("firebase-config-fields").classList.add("d-none");
    }

    // تعبئة البيانات
    document.getElementById("fb-apiKey").value = appState.firebaseConfig.apiKey || '';
    document.getElementById("fb-authDomain").value = appState.firebaseConfig.authDomain || '';
    document.getElementById("fb-databaseURL").value = appState.firebaseConfig.databaseURL || '';
    document.getElementById("fb-projectId").value = appState.firebaseConfig.projectId || '';
...
function saveSystemSettings() {
    ...
    if (mode === 'firebase') {
        const tempConfig = { apiKey, authDomain, databaseURL, projectId };
        if (!validateFirebaseConfig(tempConfig)) {
            alert("يرجى إدخال كافة حقول إعدادات Firebase المطلوبة بشكل صحيح للربط السحابي.");
            return;
        }
        appState.firebaseConfig = tempConfig;
    }

    appState.dbMode = mode;
